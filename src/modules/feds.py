...
LOGGER.info(
    "Original federation module by MrYacha, reworked by Mizukito Akito (@RealAkito) on Telegram."
)
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Failed to create federation! Head over to @tohsakas to notify us of the error."
            )
            return
...
def del_fed(bot: Bot, update: Update, args: List[str]):
    ...
    update.effective_message.reply_text(
        "Are you sure you want to delete your federation? This action cannot be reversed, you will lose your entire ban list, and '{}' will be permanently lost.".format(
            getinfo["fname"]
        ),
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please head to @tohsakas to report this."
            )
            return
...
def user_join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if user_id == get_owner:
            update.effective_message.reply_text(
                "Why are you trying to promote the federation owner?"
            )
            return
        if getuser:
            update.effective_message.reply_text(
                "This user is already an admin of the federation!"
            )
            return
        if user_id == bot.id:
...
def user_demote_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if sql.search_user_in_fed(fed_id, user_id) == False:
            update.effective_message.reply_text(
                "This user isn't even a federation admin!"
            )
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
        ...
        try:
            ...
            if excp.message in FBAN_ERRORS:
                try:
                    ...
                    LOGGER.info(
                        "Chat {} has left fed {} because bot has been kicked.".format(
                            chat, info["fname"]
                        )
                    )
                    continue
                ...
                LOGGER.warning(
                    "Cannot fban in {} because: {}".format(chat, excp.message)
                )
        except TelegramError:
            pass
    ...
    message.reply_text(
        "{} has been fbanned.".format(mention_html(user_chat.id, user_chat.first_name)),
        parse_mode=ParseMode.HTML,
    )
...
def unfban(bot: Bot, update: Update, args: List[str]):
    ...
    for chat in chat_list:
        ...
        try:
            ...
            if not x:
                message.reply_text(
                    "Un-fban failure, this user may have been un-fbanned already!"
                )
                return
        except:
            pass
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Failed to set federation rules. If this persists, reach out to us @tohsakas."
            )
            return
...
def fed_broadcast(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        ...
        if failed >= 1:
            send_text += "{} the group failed to receive the broadcast, probably because they left the federation.".format(
                failed
            )
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "rin_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="rin_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "rin_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="rin_fbanned_users.csv",
    ...
    try:
        ...
        with BytesIO(str.encode(cleantext)) as output:
            ...
            update.effective_message.reply_document(
                ...
                caption="The following is the list of users who are currently fbanned in the Federation {}.".format(
                    info["fname"]
                ),
            )
...
def fed_notif(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0] in ("yes", "on"):
            ...
            msg.reply_text(
                "Reporting turned on! Users fbanned/un-fbanned by you will be notified via PM."
            )
        elif args[0] in ("no", "off"):
            sql.set_feds_setting(user.id, False)
            msg.reply_text(
                "Reporting turned off! Users fbanned/un-fbanned will not be notified via PM."
            )
        else:
    ...
    try:
        ...
        with BytesIO(str.encode(cleantext)) as output:
            ...
            update.effective_message.reply_document(
                document=output,
                filename="fbanlist.txt",
                caption="Here is the list of all the chats in the federation {}.".format(
                    info["fname"]
                ),
            )
    ...
    if getfed:
        ...
        if delete:
            query.message.edit_text(
                "You have deleted your federation! Now all the groups that were connected with `{}` do not have a federation.".format(
                    getfed["fname"]
                ),
                parse_mode="markdown",
            )
...
def welcome_fed(bot, update):
    ...
    if fban:
        update.effective_message.reply_text(
            "This user is banned in the current federation and has been removed!"
        )
